...
run_experiment() {
    ...
    PYTHONPATH="/home/simon/otitenet" /home/simon/otitenet/.venv/bin/python -m otitenet.train_triplet_new \
        ...
        --new_size=$new_size --early_stop=$early_stop --n_trials=$n_trials > "logs/job_${i}.log" 2>> "logs/job_${i}_error.log"
        ...
        for n_calibration in 0 4; do
                ...
                for dloss in no inverseTriplet; do
                    for prototype in batch no class; do
                        if ! [[ "$prototype" == "batch" && "$dloss" == "no" ]]; then
                            ...
                            run_experiment "$model" "$fgsm" "$n_calibration" "$loss" "$dloss" "$prototype" 1 1 &
                            job_count=$(( job_count + 1 ))
                        fi
                    done
                done
            ...
            for dloss in no inverseTriplet; do
                for classif_loss in ce hinge; do
                    ...
                    run_experiment "$model" "$fgsm" "$n_calibration" "$classif_loss" "$dloss" "no" 1 1 &
                    job_count=$(( job_count + 1 ))
                done
            done
            ...
            for dloss in no inverse_softmax_contrastive inverseTriplet; do
                ...
                run_experiment "$model" "$fgsm" "$n_calibration" "softmax_contrastive" "$dloss" "no" 1 1 &
                job_count=$(( job_count + 1 ))
            done
            ...
            for loss in arcface; do
                for dloss in no inverseTriplet; do
                    ...
                    run_experiment "$model" "$fgsm" "$n_calibration" "$loss" "$dloss" "no" 1 1 &
                    job_count=$(( job_count + 1 ))
                done
            done
            ...
            for n_positives in 1; do
                for n_negatives in 1 5; do
                    for dloss in no inverseTriplet; do
                        ...
                        run_experiment "$model" "$fgsm" "$n_calibration" "triplet" "$dloss" "no" "$n_positives" "$n_negatives" &
                        job_count=$(( job_count + 1 ))
                    done
                done
            done
