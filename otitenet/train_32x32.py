...
from src.dataset import get_images_loaders
from src.utils import get_optimizer, to_categorical, get_empty_dicts, get_empty_traces, \
    log_traces, get_best_values, add_to_logger, add_to_neptune, \
    add_to_mlflow
from sklearn.model_selection import StratifiedGroupKFold, StratifiedKFold
from src.utils import get_unique_labels
from src.loggings import LogConfusionMatrix
import neptune
import mlflow
import itertools
import warnings
...
from src.SVHModule import SpatialTransformer
...
class Net(nn.Module):
    def __init__(self, device, n_cats, n_batches, is_stn=1):
        ...
        self.classifier = nn.Linear(64, n_cats).to(device)
        self.dann = nn.Linear(64, n_batches).to(device)
    ...
    def forward(self, inp):
        ...
        return enc, F.log_softmax(out, dim=1), F.log_softmax(domout, dim=1)
...
def get_images(path):
    ...
    for i, row in infos.iterrows():
        ...
        png = transforms.Resize((32, 32))(png)
        try:
            pngs += [np.array(png)]
        except:
            continue
...
class GetData:
    def __init__(self, path, args) -> None:
        ...
        self.split('test', args.groupkfold, seed=2)
...
class TrainAE:
    ...
    def train(self, params):
        ...
        if self.log_neptune:
            ...
            model["model_name"] = run["model_name"] = 'cnn'
        ...
        for h in range(self.args.n_repeats):
            ...
            for epoch in range(0, self.args.n_epochs):
                ...
                model.train()
                for group in ["all", "train"]:
                    _, _, _ = self.loop(group, optimizer_model, model, sceloss, margin, gamma, loaders[group], lists, traces)
                ...
                if np.mean(values['valid']['mcc']) > self.best_mcc:
                    ...
                    self.best_mcc = np.mean(values['valid']['mcc'])
        ...
        with torch.no_grad():
            for group in ["train", "valid", "test"]:
                closs, best_lists, traces = self.loop(group, None, model, sceloss, margin, gamma,
                                                        loaders[group], best_lists, traces)
        if self.log_neptune:
        ...
        values = log_traces(traces, values)
        self.log_rep(best_lists, best_vals, best_values, traces, model, metrics, run, loggers, model,
                        shap_model, 0, epoch)
        ...
        for group in ['train', 'valid', 'test']:
            ...
            if self.log_mlflow:
                ...
                mlflow.log_metric(f'{group}_acc',
                            metrics.accuracy_score(cats[group].argmax(1), scores[group].argmax(1)),
                            step=step)
                mlflow.log_metric(f'{group}_mcc',
                            MCC(cats[group].argmax(1), scores[group].argmax(1)),
                            step=step)
            ...
            save_roc_curve(
                ...
                binary=False,
                acc=metrics.accuracy_score(cats[group].argmax(1), scores[group].argmax(1)),
                mlops='mlflow',
                epoch=step,
                logger=None
            )
    
    def loop(self, group, optimizer_model, model, sceloss, margin, gamma, loader, lists, traces):
        """
        ...
        for i, batch in enumerate(loader):
            ...
            data, names, labels, _, old_labels, domain, _, _, pos_batch_sample, neg_batch_sample = batch
            data = data.to(self.args.device).float()
            enc, preds, dpreds = model(data)
            cats = to_categorical(labels.long(), self.n_cats).float().to(self.args.device)
            ...
            # lists[group]['dcat'] += [np.array([np.argmax(d) for d in domains.detach().cpu().numpy()])]
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_id', type=str, default='otite_new')
