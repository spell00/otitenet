...
class Dataset1(Dataset):
    def __init__(self, data, epoch, unique_labels, names=None,
                 ...
                 triplet_loss=True, prototypes_to_use='no', batch_encoder=None):
        """
        ...
        if batch_encoder is not None:
            ...
            self.batch_encoder.fit(self.unique_batches)
    ...
    def set_labels_and_batches(self, labels, batches):
        ...
        self.n_labels = {label: len(self.labels_inds[label]) for label in self.unique_labels}
        self.n_batches = {batch: len(self.batches_inds[batch]) for batch in self.unique_batches}
        self.available_labels = [label for label in self.unique_labels if self.n_labels[label] > 0]
        self.available_batches = [batch for batch in self.unique_batches if self.n_batches[batch] > 0]

    def _sample_label(self, label):
        count = self.n_labels.get(label, 0)
        if count <= 0:
            raise ValueError(f"Cannot sample label {label!r}; it has no samples in this split.")
        return self.samples[self.labels_inds[label][np.random.randint(0, count)]].copy()

    def _sample_negative_label(self, label):
        candidates = [candidate for candidate in self.available_labels if candidate != label]
        if not candidates:
            return label
    ...
    def __getitem__(self, idx):
        ...
        if self.triplet_loss:
            if self.prototypes_to_use == 'combined' and self.epoch > 0:
                ...
                try:
                    ...
                    to_rec = self._sample_label(label)

                not_label = self._sample_negative_label(label)
                try:
                    ...
                    try:
                        not_to_rec = self._sample_label(not_label)
                    except Exception:
        ...
        # Use LabelEncoder to encode batch to index
        batch_idx = self.batch_encoder.transform([batch])[0] if batch is not None else None
        return x, name, cat, label, old_label, batch_idx, to_rec, not_to_rec, pos_batch_sample, neg_batch_sample
...
class Dataset2(Dataset):
    def __init__(self, data, epoch, unique_labels, names=None,
        ...
        self.batches_inds = {g: 
            ...
            for g in ['train', 'calibration']}
        self.n_labels = {g: 
            {label: len(self.labels_inds[g][label]) for label in self.unique_labels}
            for g in ['train', 'calibration']}
        self.n_batches = {g: 
            {batch: len(self.batches_inds[g][batch]) for batch in self.unique_batches}
            for g in ['train', 'calibration']}
        self.available_labels = {
            g: [label for label in self.unique_labels if self.n_labels[g][label] > 0]
            for g in ['train', 'calibration']
        }
        self.triplet_dloss = triplet_dloss
        self.triplet_loss = triplet_loss
    ...
    def __getitem__(self, idx):
        ...
        if self.triplet_loss:
            if self.prototypes_to_use == 'combined' and self.epoch > 0:
                ...
                candidates = [candidate for candidate in self.available_labels['train'] if candidate != label]
                not_label = candidates[np.random.randint(0, len(candidates))] if candidates else label
                not_to_rec = self.samples['train'][
                    self.labels_inds['train'][not_label][np.random.randint(0, self.n_labels['train'][not_label])]
                ].copy()
        else:
