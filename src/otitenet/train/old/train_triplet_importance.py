...
def get_random_module(net, device):
    return RandConvModule(net,
                          ...
                          data_mean=[0.13854676, 0.10721603, 0.09241733],
                          data_std=[0.07892648, 0.07227526, 0.06690206],
                          clamp_output=1,
                          ).to(device)
...
def save_roc_curve(y_pred_proba, y_test, unique_labels, name, binary, acc, mlops, epoch=None, logger=None):
    ...
    if n_classes == 2:
        binary = True
        pos_col = np.where(unique_labels != 'Normal')[0]
        y_test = y_test[:, pos_col]
        y_pred_proba = y_pred_proba[:, pos_col]
    ...
    if binary:
        ...
        tpr.sort()
        fpr.sort()
        min_tpr_1 = np.min(np.argwhere(tpr == 1).flatten())
        fpr_at_tpr_1 = fpr[min_tpr_1]
        thresholds_at_tpr_1 = thresholds[min_tpr_1]
        # n_pos = y_test.sum()
        # n_neg = 1 - n_pos
        # acc_at_tpr_1 = 
        # mcc_at_tpr_1 = 
        ...
        if mlops == 'mlflow':
            ...
            mlflow.log_metric(f'{name.split("_")[0]}_fpr_at_tpr_1', fpr_at_tpr_1)
            mlflow.log_metric(f'{name.split("_")[0]}_thresholds_at_tpr_1', thresholds_at_tpr_1)
            # mlflow.log_metric('acc_at_tpr_1', acc_at_tpr_1)
            # mlflow.log_metric('mcc_at_tpr_1', mcc_at_tpr_1)
        ...
        n_classes = len(unique_labels)  # -1 to remove QC
        fpr = dict()
        tpr = dict()
        thresholds = dict()
        roc_auc = dict()
        ...
        for i in range(n_classes):
            fpr[i], tpr[i], thresholds[i] = metrics.roc_curve(label_binarize(y_test, classes=classes)[:, i], y_pred_proba[:, i])
        ...
        for i in range(n_classes):
            ...
            tpr[i].sort()
            fpr[i].sort()
            min_tpr_1 = np.min(np.argwhere(tpr[i] == 1).flatten())
            fpr_at_tpr_1 = fpr[i][min_tpr_1]
            thresholds_at_tpr_1 = thresholds[i][min_tpr_1]
            n_pos = y_test[:, i].sum()
            n_neg = 1 - n_pos
            acc_at_tpr_1 = np.round((tpr[i][min_tpr_1] * n_pos + (1-fpr[i][min_tpr_1]) * n_neg) / (n_pos + n_neg), 3)
            mcc_at_tpr_1 = np.round((1-fpr[i][min_tpr_1]) / np.sqrt((tpr[i][min_tpr_1] + fpr[i][min_tpr_1]) * (1-tpr[i][min_tpr_1]-fpr[i][min_tpr_1])), 3)
        ...
        if mlops == 'mlflow':
            mlflow.log_figure(fig, name)
            # mlflow.log_metric('fpr_at_tpr_1', fpr_at_tpr_1)
            # mlflow.log_metric('thresholds_at_tpr_1', thresholds_at_tpr_1)
            # mlflow.log_metric('acc_at_tpr_1', acc_at_tpr_1)
            # mlflow.log_metric('mcc_at_tpr_1', mcc_at_tpr_1)
...
class TrainAE:
    ...
    def train_classifier(self, lists, traces, params):
        if self.args.classifier == 'knn':
            self.classifier = KNN(n_neighbors=params['n_neighbors'], metric=params['knn_metric'])
        ...
        self.classifier.fit(train_encs, train_cats)
        preds = self.classifier.predict(train_encs)
        proba = self.classifier.predict_proba(train_encs)
    ...
    def make_weights(self, is_transform, triplet_dloss, random_recs=False):
        normalize_flag = str(self.args.normalize).lower() in ['yes', 'true', '1']
        ...
        if normalize_flag:
            train_ops.append(PerImageNormalize())
            eval_ops.append(PerImageNormalize())
    ...
    def train(self, params):
        ...
        for h in range(self.args.n_repeats):
            ...
            if self.args.classifier == 'osir':
                model = Net(self.args.device, 1, self.n_batches, model_name=self.args.model_name, is_stn=self.args.is_stn)
            else:
            ...
            for epoch in range(0, self.args.n_epochs):
                ...
                if self.args.classifier in ['knn', 'linearsvc', 'logreg']:
                    lists, traces = self.train_classifier(lists, traces, params)
                elif self.args.classifier == 'osir':
                    self.class_mean_vectors = {label: torch.Tensor(np.mean(np.concatenate(lists['train']['encoded_values'])[np.concatenate(lists['train']['labels']) == label], axis=0)).to(self.args.device) for label in self.unique_labels}
                model.eval()
                for group in ["valid", "test"]:
                ...
                if values['valid']['mcc'][-1] > self.best_mcc:
                    ...
                    if self.args.classifier in ['knn', 'linearsvc', 'logreg']:
                        filehandler = open(f'{self.complete_log_path}/{self.args.classifier}.pickle',"wb")
                        pickle.dump(self.classifier, filehandler)
                        filehandler.close()
        ...
        # shap_model.eval()
        if self.args.classifier in ['knn', 'linearsvc', 'logreg']:
            filehandler = open(f'{self.complete_log_path}/{self.args.classifier}.pickle',"rb")
            self.classifier = pickle.load(filehandler)
            filehandler.close()
    ...
    def loop(self, group, optimizer_model, model, params, loader, lists, traces):
        ...
        for i, batch in enumerate(loader):
            ...
            elif self.args.classif_loss == 'osir':
                not_to_rec = not_to_rec.to(self.args.device).float()
                pos_enc, _ = model(to_rec)
                neg_enc, _ = model(not_to_rec)
                pos_prob = model.classifier(self.l1loss(enc, pos_enc))
                neg_prob = model.classifier(self.l1loss(enc, neg_enc))
                loss1 = self.bceloss(pos_prob, torch.ones_like(pos_prob))
                loss2 = self.bceloss(neg_prob, torch.zeros_like(neg_prob))
                classif_loss = loss1 + loss2
            elif self.args.classif_loss == 'ce':
                classif_loss = self.celoss(model.classifier(enc), labels.long().to(self.args.device))
    ...
    def predict(self, group, model, loader, lists, traces):
        ...
        for i, batch in enumerate(loader):
            ...
            enc, dpreds = model(data)
            if self.args.classifier in ['knn', 'linearsvc', 'logreg']:
                preds = self.classifier.predict(enc.view(enc.shape[0], -1).detach().cpu().numpy())
                proba = self.classifier.predict_proba(enc.view(enc.shape[0], -1).detach().cpu().numpy())
            elif self.args.classifier == 'nn':
                ...
                for datum in enc:
                    similarities = []
                    for label in self.unique_labels:
                        tmp = model.classifier(self.l1loss(datum.view(1, -1), self.class_mean_vectors[label].view(1, -1)))[0]
                        similarities += [tmp]
                    similarities = torch.stack(similarities)
                    proba = torch.softmax(similarities, 0)
                    preds += [torch.argmax(proba).item()]
                    probs += [torch.max(proba).item()]
                    probas += [proba.detach().cpu().numpy()[:, 0]]
                proba = np.stack(probas)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--classifier', type=str, default='nn', help='')
