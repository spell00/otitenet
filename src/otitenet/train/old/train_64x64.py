...
class TrainAE:
    ...
    def train(self, params):
        ...
        if self.log_neptune:
            ...
            model["dataset_name"] = run["dataset_name"] = 'otite'
        else:
            model = None
            run = None
        ...
        for h in range(self.args.n_repeats):
            ...
            if self.args.n_calibration > 0:
                ...
                for group in ['valid', 'test']:
                    calib_inds = np.random.choice(list(range(len(self.data['labels'][group]))), self.args.n_calibration, replace=False)
                    # add them to train
        ...
        if np.mean(best_mccs) > self.best_best_mcc:
            try:
                if os.path.exists(
                        f'logs/best_models/model_classifier/otitenet'):
                    shutil.rmtree(
                        f'logs/best_models/model_classifier/otitenet',
                        ignore_errors=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--exp_id', type=str, default='64x64')
